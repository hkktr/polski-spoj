 ...
 private:
  ...
  std::vector<int> ConvertBase(const std::vector<int>& vector,
    ...
    std::vector<long long> bases(
        static_cast<std::size_t>(std::max(old_base_digits, new_base_digits)) +
        1);
    bases[0] = 1;
    for (std::size_t i{1}; i < bases.size(); ++i) bases[i] = bases[i - 1] * 10;
  ...
  std::vector<long long> KaratsubaMultiply(
    ...
    for (std::size_t i{0}; i < k; ++i) a2[i] += a1[i];
    for (std::size_t i{0}; i < k; ++i) b2[i] += b1[i];

    vll_t temp{KaratsubaMultiply(a2, b2)};
    for (std::size_t i{0}; i < a1b1.size(); ++i) temp[i] -= a1b1[i];
    for (std::size_t i{0}; i < static_cast<int>(a2b2.size()); ++i)
      temp[i] -= a2b2[i];

    for (std::size_t i{0}; i < temp.size(); ++i) result[i + k] += temp[i];
    for (std::size_t i{0}; i < a1b1.size(); ++i) result[i] += a1b1[i];
    for (std::size_t i{0}; i < a2b2.size(); ++i) result[i + n] += a2b2[i];
...
void PrintPascalRow(int n) {
  ...
  for (int i{1}; i <= n; ++i) {
    BigInt current{(previous * (n - i + 1)) / i};
    std::cout << " " << current;
